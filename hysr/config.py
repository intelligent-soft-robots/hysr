...
def _import_packages(label: str, config: typing.Dict[str, typing.Any]) -> None:
    """
    ...
    if not isinstance(imports, collections.abc.Iterable):
        raise ValueError(
            f"{label}: the toml configuration has an unexpected value for imports (should be iterable of string)"
        )
    for import_ in imports:
        try:
            importlib.import_module(str(import_))
...
def read_factory_class(
    label: str, d: typing.Dict[str, typing.Any], imports: typing.Iterable[str] = ()
    ...
    Imports is a list of names of packages to import before evaluating the args and kwargs
    """
    ...
    for import_ in imports:
        try:
            ...
            raise ValueError(f"failed to import module {import_}: " f"{e}")
